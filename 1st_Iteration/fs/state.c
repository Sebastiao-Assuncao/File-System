...
/* mutex used for syncronization */
pthread_mutex_t fsMutex = PTHREAD_MUTEX_INITIALIZER;

/* rwlock used for syncronization */
pthread_rwlock_t fsRwlock = PTHREAD_RWLOCK_INITIALIZER;
...
/*
 * Locks the FileSystem's locks using the sync strategy given by the user.
 * 
 * Inputs:
 *  - rw: 0 -> write section; 1 -> read only section (only used in rwlock strategy)
 */
void lock(char rw) {
    /* strategy nosync -> no lock is done */
    if (syncStrategy == 'n') {
        return;
    }
    /* strategy mutex -> mutex_lock is done and validated */
    if (syncStrategy == 'm') {
        if(pthread_mutex_lock(&fsMutex) == 0)
            return;

        fprintf(stderr, "Error: not able to lock mutex.\n");
        exit(EXIT_FAILURE);
    }
    /* strategy rwlock -> check if is for read only (rw = 1) or for read write (rw = 0), locks and validates*/
    if (syncStrategy == 'r') {
        if (rw == 0)
        ...
        fprintf(stderr, "Error: not able to lock rwlock.\n");
        exit(EXIT_FAILURE);
    }
...
/*
 * Unlocks the FileSystem's locks using the sync strategy given by the user.
 */
void unlock(){
    /* strategy nosync -> no unlock is done */
    if (syncStrategy == 'n') {
        return;
    }
    /* strategy mutex -> mutex_unlock is done and validated */
    if (syncStrategy == 'm') {
        if(pthread_mutex_unlock(&fsMutex) == 0)
            return;

        fprintf(stderr, "Error: not able to unlock mutex.\n");
        exit(EXIT_FAILURE);
    }
    /* strategy rwlock -> rw_unlock is done and validates*/
    if (syncStrategy == 'r') {
        if(pthread_rwlock_unlock(&fsRwlock) == 0)
            return;
            
        fprintf(stderr, "Error: not able to unlock rwlock.\n");
        exit(EXIT_FAILURE);
    }
